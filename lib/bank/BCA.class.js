...
class ScrapBCA extends ScraperBank {
    ...
    async launchAndSetupBrowser() {
        ...
        console.log("[ LOG ] ["+this.user+"] Starting ..")
...
async loginToBCA(page) {
    ...
    page.on("dialog", async dialog => {
        await dialog.accept();
        console.log("[ LOG ] ["+this.user+"] "+dialog.message());
        this.loginError = dialog.message();  
    });
    console.log("[ LOG ] ["+this.user+"] ["+this.user+"] Login to BCA .. ")
    await page.type(BCASelectors.LOGIN_PAGE.userField, this.user, { delay: 200 });
...
async processGetBalance(page) {
    ...
    await newPage.waitForSelector(BCASelectors.SETTLEMENT_PAGE.startDateField);
        ...
        console.log("[ LOG ] ["+this.user+"] Get mutasi ..")
        await newPage.click(BCASelectors.SETTLEMENT_PAGE.submitButton, { delay: 2000 });
    ...
    async getBalance(tglawal, blnawal, tglakhir, blnakhir) {
        ...
        try {
            ...
            console.log("[ LOG ] ["+this.user+"] Success get Balance for ("+this.user+")");
            ...
            console.log("[ LOG ] ["+this.user+"] "+error);
                await this.logoutAndClose(page);
                return {status:false,
        ...
        async getStatement(tglawal, blnawal, tglakhir, blnakhir) {
            ...
            this.dialogMessage = null;  // Initialize dialogMessage
                ...
                try {
                    ...
                    console.log("[ LOG ] ["+this.user+"] Success get mutasi for ("+this.user+")"+" Jumlah mutasi (" + resultsettlement.length+")");
                    const exists = await this.checkIfReturnToLogin(newPage, BCASelectors.LOGIN_PAGE.userField);
    ...
    async logoutAndClose(page) {
        try {
            console.log("[ LOG ] ["+this.user+"] Logout..");
